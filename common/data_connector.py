...
class DataConnector:
    ...
    def updateStatus(self, model_id, status, reason, report):
        # prepare update query and data
        query = "UPDATE datagov.ml_model_instance " \
                "SET status = %s, reason = %s, report = %s " \
                "WHERE id  = %s"

        args = (status, reason, model_id, report)
    ...
    def updateValidationStatus(self, model_id, status, reason, report):
        # prepare update query and data
        query = "UPDATE datagov.ml_model_validation_instance " \
                "SET status = %s, reason = %s, report = %s " \
                "WHERE id  = %s"
        print(query)
        args = (status, reason, report, model_id)
