...
def train(id, model, reqdata):
    ...
    if(response["status"]==1):
        ...
        data_connector.updateStatus(id,response.status,response.reason,'')
    #update model status
...
def validate(trainId, validationId, model, reqdata):
    ...
    data_connector.updateValidationStatus(validationId,response["status"],response["reason"],json.dumps(response["report"]))
    #update model status
